  ...
  return (
    ...
    >
      <div
        ...
        style={{
          opacity,
          background: `radial-gradient(600px circle at ${position.x}px ${position.y}px, rgba(${
            isDev ? '168,85,247' : '59,130,246'
          },0.15), transparent 40%)`,
        }}
      />
      <div className="relative z-10 flex flex-col h-full">
        <div
          className={`mb-4 inline-flex h-10 w-10 items-center justify-center rounded-lg group-hover:scale-110 transition-transform duration-300
...
const TerminalSim: React.FC = () => {
  const [lines, setLines] = useState<Array<{ text: string; color?: string }>>([
    { text: '> NFTPulse Module [SNIPER] initialized' },
    { text: '> Listening to mempool...' },
  ]);

  useEffect(() => {
    const sequence: Array<{ text: string; delay: number; color?: string }> = [
      { text: '> Target found: 0x7a...4f9', delay: 800 },
      { text: '> Gas spike detected (50 gwei)', delay: 1500, color: 'text-yellow-400' },
      { text: '>> MINT SUCCESSFUL', delay: 2500, color: 'text-green-400 font-bold' },
    ];
    ...
    sequence.forEach(({ text, delay, color }) => {
      const t = setTimeout(() => {
        setLines((prev) => [...prev.slice(-5), { text, color }]);
      }, delay);
      timeouts.push(t);
    });
  ...
  return (
    <div className="font-mono text-xs p-4 bg-black/90 rounded-lg border border-white/5 shadow-2xl h-40 overflow-hidden flex flex-col justify-end">
      {lines.map((l, i) => (
        <div key={i} className={l.color ?? 'text-gray-400'}>
          {l.text}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#02040a] text-white font-sans selection:bg-blue-500/30">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-purple-600/10 rounded-full blur-[120px] mix-blend-screen" />
      ...
      <div className="relative z-10 pt-32 pb-20 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-2 gap-6 h-auto">
          <BentoCard
            ...
            subtitle="Advanced mempool monitoring. Executes transactions seconds after liquidity detection."
            icon={Crosshair}
            size="lg"
            delay={100}
          >
          ...
          <BentoCard
            title="Vault Manager"
            subtitle="Orchestrate multiple wallets. Mass-mint, consolidate, and scatter funds with one command."
            icon={Shield}
            delay={200}
          >
            <div className="flex -space-x-3 mt-4">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-10 h-10 rounded-full bg-gray-800 border border-gray-600 flex items-center justify-center text-xs font-mono"
                >
                  W{i}
                </div>
              ))}
              <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-xs font-bold shadow-lg">
                +
              </div>
            </div>
          </BentoCard>
          ...
          <BentoCard
            title="Direct Contract Intel"
            subtitle="Talk directly to contracts (SeaDrop, marketplaces & custom drops) without relying on slow front-ends."
            icon={Zap}
            size="tall"
            delay={300}
          >
          ...
          <BentoCard
            ...
            subtitle="Your keys stay local. Encrypted storage and execution from your own environment."
            icon={Database}
            delay={400}
          >
          ...
          <BentoCard
            ...
            subtitle="More modules are being forged with the community. Expect frequent iterations instead of one big launch."
            icon={FlaskConical}
            size="lg"
            delay={500}
            isDev
